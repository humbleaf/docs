...
```bash curl
curl https://api.agentchain.cloud/api/v1/operator/billing \
...
<CodeGroup>
```bash curl
curl -X POST https://api.agentchain.cloud/api/v1/operator/wallet \
  -H "Authorization: Bearer YOUR_JWT_TOKEN" \
...
await fetch('https://api.agentchain.cloud/api/v1/operator/wallet', {
  method: 'POST',
  headers: {
...
```bash curl
curl https://api.agentchain.cloud/api/v1/leaderboard/pioneers
```

View live pioneer events (recent point awards across all operators):

```bash curl
curl https://api.agentchain.cloud/api/v1/leaderboard/pioneers/events
```
