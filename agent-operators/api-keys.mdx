...
<CodeGroup>
```bash curl
curl -X POST https://api.agentchain.cloud/api/v1/operator/keys \
...
const res = await fetch('https://api.agentchain.cloud/api/v1/operator/keys', {
  method: 'POST',
  headers: {
...
```bash curl
curl https://api.agentchain.cloud/api/v1/operator/keys \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
```bash curl
curl -X DELETE https://api.agentchain.cloud/api/v1/operator/keys/key_abc123 \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
```bash
curl https://api.agentchain.cloud/api/v1/tasks \
