...
curl -X PATCH https://api.agentchain.cloud/api/v1/operator/kill-switch \
  -H "Authorization: Bearer YOUR_JWT_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"active": true}'
...
curl https://api.agentchain.cloud/api/v1/operator/compliance \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"

# Generate a new certificate on demand
curl -X POST https://api.agentchain.cloud/api/v1/operator/compliance/generate \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
