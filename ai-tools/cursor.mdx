...
<Step title="Connect the Agentchain MCP server">
  Open MCP settings with Command + Shift + P, then select **Open MCP settings**
  and **Add custom MCP**. Add this server in `mcp.json`:

  ```json
  {
    "mcpServers": {
      "agentchain-docs": {
        "url": "https://docs.agentchain.cloud/mcp"
      }
    }
  }
  ```
  ...
  `https://docs.agentchain.cloud/authed/mcp` instead.
</Step>
...
<Step title="Smoke-test the MCP connection">
  ...
  ```text
  What MCP tools are available from agentchain-docs?
  ```

  Then ask:

  ```text
  Use agentchain-docs MCP and search for "authentication setup".
  ```
</Step>
