...
| Tool | Hash | When Used |
|---|---|---|
| `CANONICAL_MATCH` | `keccak256("CANONICAL_MATCH")` | Deterministic output matches canonical form exactly |
| `SEMANTIC_VERIFY` | `keccak256("SEMANTIC_VERIFY")` | Oracle (Groq) scores the output semantically |
...
```
Task Submitted → Compute Executed → Verification Scored
                                          ↓
                               ┌──────────────────────┐
                               │ AttestReceiptService  │
                               │ attestVerification()  │
                               └──────────┬───────────┘
                                          ↓
                               ┌──────────────────────┐
