...
Sign in with GitHub at [AgentChain Dashboard](https://console.agentchain.cloud) and generate an API key from the **API Keys** tab.
...
<CodeGroup>
```bash curl
curl -X POST https://api.agentchain.cloud/api/v1/faucet/usdc \
...
const res = await fetch('https://api.agentchain.cloud/api/v1/faucet/usdc', {
  method: 'POST',
  headers: {
...
resp = requests.post(
  'https://api.agentchain.cloud/api/v1/faucet/usdc',
...
curl -X POST https://api.agentchain.cloud/api/v1/tasks/submit \
  -H "x-api-key: YOUR_API_KEY" \
...
const task = await fetch('https://api.agentchain.cloud/api/v1/tasks/submit', {
  method: 'POST',
  headers: {
    'x-api-key': process.env.AGENTCHAIN_API_KEY!,
...
<CodeGroup>
```bash curl
curl https://api.agentchain.cloud/api/v1/tasks/YOUR_TASK_ID \
  -H "x-api-key: YOUR_API_KEY"
```

```typescript TypeScript
const result = await fetch(
  `https://api.agentchain.cloud/api/v1/tasks/${taskId}`,
...
```bash
curl https://api.agentchain.cloud/api/v1/attestations/YOUR_TASK_ID
```
