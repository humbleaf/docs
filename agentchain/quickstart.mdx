...
Pass the `paymentAddressTxHash` from Step 2 as the `X-402-Payment` header (Base64-encoded JSON). The faucet response includes a pre-built header value in `instructions`.
...
```
Your wallet  →  faucet mints MockUSDC  →  payment gate funded
     ↓
POST /tasks/submit + X-402-Payment header
     ↓
X-402 interceptor verifies on-chain transfer  →  task created
     ↓
Compute provider executes task  →  result pinned to IPFS
     ↓
Verification oracle scores the result
     ↓
Result anchored on-chain (TaskMarket)  →  EAS attestation minted
     ↓
attestationUid + easscanUrl returned in task response
```
...
<CardGroup cols={2}>
  ...
  >
    Full task submission schema, response types, and all 50 endpoints.
  </Card>
</CardGroup>