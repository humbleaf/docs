...
```mermaid
flowchart TD
    A["Agent submits action<br/>(SDK / Trigger.dev / ElizaOS)"] --> B["Gateway evaluates policy<br/>(Stage 1.5c)"]
    B -->|Below threshold| C["Pipeline continues<br/>(stages 2–8)"]
    C --> D["✅ Receipt issued"]
    B -->|Above threshold| E["PENDING_APPROVAL"]
    E --> F{"Auto-approve<br/>rules pass?"}
    F -->|Yes| G["Auto-release"]
    G --> C
    F -->|No| H["Queue for human review"]
    H --> I["Operator resolves<br/>(Dashboard / API / Webhook)"]
    I -->|Approved| C
    I -->|Denied| J["❌ Task FAILED<br/>No settlement consumed"]
    ...
    style G fill:#0a0a0b,stroke:#10B981,stroke-width:2px,color:#fafafa
    style H fill:#0a0a0b,stroke:#5ba3b5,stroke-width:2px,color:#fafafa
    style I fill:#0a0a0b,stroke:#818cf8,stroke-width:2px,color:#fafafa
    style J fill:#0a0a0b,stroke:#fbbf24,stroke-width:2px,color:#fafafa
```
...
```bash
curl -X PATCH https://api.agentchain.cloud/api/v1/operator/guardrails \
...
```bash
curl -X PATCH https://api.agentchain.cloud/api/v1/approvals/$APPROVAL_ID \
  -H "x-api-key: $API_KEY" \
  -H "Content-Type: application/json" \
...
```json
{
  ...
  "resolveUrl": "https://console.agentchain.cloud/approve/clx9abc123",
  "decision": "approved",
...
```bash
curl -X POST https://api.agentchain.cloud/api/v1/approvals/$APPROVAL_ID/callback \
  -H "x-api-key: $API_KEY" \
  -H "Content-Type: application/json" \
