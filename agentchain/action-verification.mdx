...
| Property | Action Verification | Inference Verification |
|---|---|---|
| **Determinism** | Binary yes/no | Probabilistic score |
| **Proof type** | State root comparison | Semantic similarity |
| **Can be disputed** | Yes (BisectionCourt) | Yes (but subjective) |
| **Cost** | ~$0.001 gas on Base | ~$0.003 Groq API call |
| **Use case** | Token swaps, contract calls, state mutations | Text generation, summarization, analysis |
...
| `toolId` | `keccak256("CANONICAL_MATCH")` or `keccak256("SEMANTIC_VERIFY")` |
